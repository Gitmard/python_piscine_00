...
def test_ft_exercise(exercise_folder, exercise_file_name):
    """
    This function tries to run one of your exercises.

    For example: test_ft_exercise("ex1", "ft_plot_area") will:
    - Look for a file called ex1/ft_plot_area.py
    - Import it
    - Call the function ft_plot_area() inside it
    """
    print(f"\n=== Testing {exercise_folder}/{exercise_file_name} ===")

    try:
        # Add the exercise folder to Python's path temporarily
        exercise_path = Path(exercise_folder)
        if not exercise_path.exists():
            print(f"❌ Could not find folder {exercise_folder}/")
            print("   Make sure the folder exists in the same directory as \
                main.py")
            return

        # Add the folder to sys.path if not already there
        folder_str = str(exercise_path.resolve())
        ...
        # Remove the folder from sys.path to avoid conflicts
        if folder_str in sys.path:
            sys.path.remove(folder_str)

    except ImportError as error:
        print(f"❌ Could not find {exercise_file_name}.py in \
            {exercise_folder}/")
        print(f"   Make sure your file exists: {exercise_folder}/\
            {exercise_file_name}.py")
